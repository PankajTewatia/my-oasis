...
public class RedisServiceVertxEBProxy implements RedisService {
  ...
  public RedisService persistUserInfo(String email, UserInfo userInfo, Handler<AsyncResult<UserInfo>> resultHandler){
    ...
    JsonObject _json = new JsonObject();
    _json.put("email", email);
    _json.put("userInfo", userInfo != null ? userInfo.toJson() : null);

    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "persistUserInfo");
    _deliveryOptions.getHeaders().set("action", "persistUserInfo");
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
  ...
  public RedisService persistSourceInfo(String sourceId, EventSource eventSource, Handler<AsyncResult<EventSource>> resultHandler){
    ...
    JsonObject _json = new JsonObject();
    _json.put("sourceId", sourceId);
    _json.put("eventSource", eventSource != null ? eventSource.toJson() : null);

    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "persistSourceInfo");
    _deliveryOptions.getHeaders().set("action", "persistSourceInfo");
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
  ...
  public RedisService deleteKey(String key, Handler<AsyncResult<Boolean>> resultHandler){
    ...
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "deleteKey");
    _deliveryOptions.getHeaders().set("action", "deleteKey");
