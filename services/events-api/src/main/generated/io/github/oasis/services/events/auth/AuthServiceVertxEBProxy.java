...
public class AuthServiceVertxEBProxy implements AuthService {
  ...
  public AuthService readSource(String sourceId, Handler<AsyncResult<EventSource>> handler){
    ...
    _deliveryOptions.addHeader("action", "readSource");
    _deliveryOptions.getHeaders().set("action", "readSource");
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        handler.handle(Future.failedFuture(res.cause()));
      } else {
        handler.handle(Future.succeededFuture(res.result().body() != null ? new io.github.oasis.services.events.model.EventSource((JsonObject)res.result().body()) : null));
      }
    });
    return this;
  }
